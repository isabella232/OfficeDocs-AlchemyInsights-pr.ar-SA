---
title: اختبار الصور المضمنة الخيمياء
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: a256e8680379c006287ae30996763051c4d9ff85
ms.translationtype: MT
ms.contentlocale: ar-SA
ms.lasthandoff: 09/04/2019
ms.locfileid: "36743116"
---
# <a name="access-the-domains-page"></a>الوصول إلى صفحة المجالات

للوصول إلى صفحة المجالات، ما زال فوق هذا الرمز أدناه:

  
<img src='data:image/png;base64,'/>

ثم يمكنك إضافة المجال أو تعديله أو إزالته.
لمزيد من المعلومات، راجع الأسئلة المتداولة حول [المجالات.](https://docs.microsoft.com/office365/admin/setup/domains-faq)
  