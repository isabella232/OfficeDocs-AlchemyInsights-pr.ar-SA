---
title: تنظيم البريد الإلكتروني الخاص بك في Outlook على ويب
ms.author: daeite
author: daeite
manager: joallard
ms.date: 6/14/2019
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: f863adb782abac77c45336ad68a99006e070ee09
ms.sourcegitcommit: 1d98db8acb9959aba3b5e308a567ade6b62da56c
ms.translationtype: MT
ms.contentlocale: ar-SA
ms.lasthandoff: 08/22/2019
ms.locfileid: "36556732"
---
# <a name="organize-your-email"></a>تنظيم البريد الإلكتروني الخاص بك

في Outlook على الويب، يمكنك استخدام الأوامر مثل **كنس** و **الانتقال إلى** حذف أو نقل الرسائل الواردة من مرسل معين تلقائياً. تظهر هذه الأوامر أعلى قائمة الرسائل عند تحديد رسالة في علبة الوارد الخاصة بك.

- **كنس** <img src='data:image/png;base64,' />
 يتيح لك حذف الرسائل في علبة الوارد. يمكنك تلقائياً حذف كافة رسائل البريد الإلكتروني الواردة من هذا المرسل، والاحتفاظ بالبريد الإلكتروني فقط أحدث وحذف بقية، أو حذف البريد الإلكتروني أقدم من 10 أيام.
- **الانتقال إلى** <img src='data:image/png;base64,' /> يتيح لك نقل رسالة رسالة محددة إلى مجلد آخر.

لمزيد من المعلومات، راجع [العمل مع مجلدات الرسائل في Outlook على الويب](https://support.office.com/article/ae0f10d6-54e7-4f29-acd3-78cdc3fdcb9f).