---
title: شراء تراخيص إضافية
ms.author: cmcatee
author: cmcatee-MSFT
manager: mnirkhe
ms.date: 3/2/2018
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Priority
ms.collection: Adm_O365
ms.custom: Adm_O365
ms.assetid: 75fdfd6d-cae7-40fd-bc50-8674bcd3a771
ms.openlocfilehash: 41d5d191ae78ffe8073898d6856c2d39a54ed310
ms.sourcegitcommit: d6ea5e9458a2b8ceaab3ac4bd483e1130b9a398a
ms.translationtype: MT
ms.contentlocale: ar-SA
ms.lasthandoff: 01/15/2019
ms.locfileid: "28272998"
---
# <a name="buy-additional-licenses"></a>شراء تراخيص إضافية

1. في مركز الإدارة، انتقل إلى صفحة [الاشتراك](https://go.microsoft.com/fwlink/p/?linkid=842054) ، أو اختر **الفواتير** \> **الاشتراكات**.
    
2. في الصفحة **اشتراكات** ، الاشتراك الذي تريد إضافة تراخيص إلى، ثم اختر و **تراخيص إضافة/إزالة**.
    
    [لا أرى ما إذا كان الارتباط إضافة/إزالة؟](https://support.office.com/article/36081d8d-b3fa-4948-8c34-e217bba825e1#bkmk_no_link)
    
3. في المربع **إجمالي التراخيص** ، أدخل عدد التراخيص التي تحتاج لهذا الاشتراك ثم قم باختيار **إرسال** \> **إغلاق**. على سبيل المثال، إذا كان لديك 100 تراخيص وتحتاج إلى إضافة مزيد من 5، أدخل 105.
    
    > [!NOTE]
    > إذا كنت تدفع مقابل الاشتراك ببطاقتك الائتمانية أو حسابك البنكي، تتوفر أي تراخيص جديدة تشتري مباشرة بعد أن تتلقى تأكيدا بترتيب. إذا كان الدفع بالفاتورة، قد تحتاج إلى الانتظار لفحص ائتمان قبل أن تتوفر لديك تراخيص جديدة لاستخدامها. 
  
 **موارد إضافية**
  
- [شراء التراخيص الخاصة بك Office 365](https://support.office.com/article/36081d8d-b3fa-4948-8c34-e217bba825e1)
    
- [تعيين التراخيص للمستخدمين في Office 365 للشركات](https://support.office.com/article/997596b5-4173-4627-b915-36abac6786dc)
    

